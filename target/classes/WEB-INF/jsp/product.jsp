...
<img class = "sidebar" src="/img/<%= product.getPicture()%>" width="560" height="445"><br/>
    <form method="post" name="1" action="/add-product" enctype="multipart/form-data">
        ...
        <input id="1" type="text" name="name" value="<%= product.getName()%>"><br/>
        <label for="2">Описание </label>
        <textarea id="2" type="text" name="about" value=><%= product.getAbout()%></textarea><br/>
        <label for="3">Производитель </label>
        <input id="3" type="text" name="manufacturer" value="<%= product.getManufacturer()%>"><br/>
        <label for="4">Картинка </label>
        <input id="4" type="file" value="<%= product.getPicture()%>" name="picture" ><br/>
